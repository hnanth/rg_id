...
if [ -z "$INPUT_FILE" ]; then
	...
	echo "Script used for uniprot_receptor_domain_idp"
	echo "Usage: $0 <filename> <fasta_file> [--zip]"
...
if [ ! -f "$INTERFACE" ]; then
	...
	python annotate_iface_af.py "$COLABFOLD_DIR" "$PYROSETTA_SCORES" "$INTERFACE" --n_workers 10 --idp_chain "B" --receptor_chain "A" $PYTHON_FLAG
else
...
if [ ! -f "$ELECTROSTATICS" ]; then
	...
	python annotate_electrostatics.py "$COLABFOLD_DIR" "$INTERFACE" "$ELECTROSTATICS" --n_workers 10 --idp_chain "B" --receptor_chain "A" $PYTHON_FLAG
else
